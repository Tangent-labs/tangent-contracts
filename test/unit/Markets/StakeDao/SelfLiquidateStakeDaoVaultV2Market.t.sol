...
contract SelfLiquidateStakeDaoVaultV2Market is MarketDeploymentContext {
    ...
    function test_selfLiquidate_stakeDao_vault_v2_with_isReceipt_to_true_should_fail() external {
        vm.startPrank(usr1);
        uint256 usgToRepay = 1_000 ether;
        uint256 collatToLiquidate = 1_200 ether;

        vm.expectRevert(abi.encodeWithSelector(MarketCore.NoZapLiquidateWithReceipt.selector));
        market.selfLiquidate(
            ...
            ZapStruct({router: address(usg), routerCall: ""})
        );
    }
}
