...
contract MigratoorCurveLPToCurveLP is MarketDeploymentContext {
    ...
    function test_full_repay_transfer_0_debt() external {
        ...
        assertEq(marketViewer.userDebt(marketFrom, usr1), 0);
        assertEq(marketViewer.userDebt(marketTo, usr1), debtIn);

        assertEq(marketFrom.totalCollateral(), collatIn - collatToWithdraw);
        assertApproxEqRel(marketTo.totalCollateral(), collatIn + collatToWithdraw, 1e16);

        assertEq(marketViewer.totalDebt(marketFrom), 0);
        assertEq(marketViewer.totalDebt(marketTo), debtIn);

        assertEq(marketFrom.cvxRewardToken().balanceOf(address(marketFrom)), collatIn - collatToWithdraw);
        assertApproxEqRel(marketTo.cvxRewardToken().balanceOf(address(marketTo)), collatIn + collatToWithdraw, 1e16);
    }
    ...
    function test_partial_migrate() external {
        ...
        assertEq(marketViewer.userDebt(marketFrom, usr1), debtIn - debtToRemove);
        assertEq(marketViewer.userDebt(marketTo, usr1), debtToRemove - debtToRepay);

        assertEq(marketFrom.totalCollateral(), collatIn - collatToWithdraw);
        assertApproxEqAbs(marketTo.totalCollateral(), collatToWithdraw, 50 ether);

        assertEq(marketViewer.totalDebt(marketFrom), debtIn - debtToRemove);
        assertEq(marketViewer.totalDebt(marketTo), debtToRemove - debtToRepay);
    ...
    function test_migrate_only_collateral() external {
        ...
        assertEq(marketViewer.userDebt(marketFrom, usr1), debtIn);
        assertEq(marketViewer.userDebt(marketTo, usr1), 0);
        ...
        assertEq(marketViewer.totalDebt(marketFrom), debtIn);
        assertEq(marketViewer.totalDebt(marketTo), 0);
    ...
    function test_migrate_only_debt() external {
        ...
        assertEq(marketFrom.collateralBalances(usr1), collatIn);
        assertEq(marketTo.collateralBalances(usr1), 100_000 ether);

        assertEq(marketViewer.userDebt(marketFrom, usr1), debtIn - 10_000 ether, "Check user debt on Market From");
        assertEq(marketViewer.userDebt(marketTo, usr1), 80_000 ether + 10_000 ether, "Check user debt on Market To");

        assertEq(marketFrom.totalCollateral(), collatIn - 0);
        assertApproxEqAbs(marketTo.totalCollateral(), 0, 100_000 ether);

        assertEq(marketViewer.totalDebt(marketFrom), debtIn - 10_000 ether);
        assertEq(marketViewer.totalDebt(marketTo), 80_000 ether + 10_000 ether);

        assertEq(marketFrom.cvxRewardToken().balanceOf(address(marketFrom)), collatIn);
        assertEq(marketTo.cvxRewardToken().balanceOf(address(marketTo)), 100_000 ether);
    }
}
