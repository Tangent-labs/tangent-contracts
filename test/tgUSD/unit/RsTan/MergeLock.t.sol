...
contract MergeLock is MarketDeploymentContext {
    ...
    function test_merge_tokenA_endlock_bigger_than_tokenB() external {
        vm.startPrank(usr1);
        (uint48 endLockBefore, uint208 amount3Before) = rsTanService.locks(3);
        rsTanService.merge(3, 2);
        (uint48 endLockAfter, uint208 amountMerged) = rsTanService.locks(3);
    ...
    function test_merge_tokenB_endlock_bigger_than_tokenA() external {
        vm.startPrank(usr1);
        (uint48 endLock2, uint208 amount2Before) = rsTanService.locks(2);
        rsTanService.merge(2, 3);
        (uint48 endLockAfter, uint208 amountMerged) = rsTanService.locks(2);

        assertEq(amountMerged, 2 * amount);
        assertEq(endLock2 + 1 weeks, endLockAfter);

        vm.expectRevert(abi.encodeWithSignature("ERC721NonexistentToken(uint256)", 3));
        rsTanERC721.ownerOf(3);
    }
}
