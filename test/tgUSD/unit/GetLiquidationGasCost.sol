...
contract GetLiquidationGasCost is TgUSDDeployContext {
    ...
    function _getCoinsAndLP() internal returns (uint256) {
        ...
        uint256 lpAmount = lp.add_liquidity([amount0, amount1], 0); // 118k
        vm.stopPrank();
        return lpAmount;
    }
    ...
    //     vm.startPrank(usr1);
    //     lp.approve(address(AddrGlobal.CVX_BOOSTER), 1_000_000_000_000 ether); // Not taken into account because approval will be already done
    //     AddrGlobal.CVX_BOOSTER.deposit(poolId, lpAmount, false); // 403 k
    //     vm.stopPrank();
    // }
    ...
    //     vm.startPrank(usr1);
    //     lp.approve(address(AddrGlobal.CVX_BOOSTER), 1_000_000_000_000 ether); // Not taken into account because approval will be already done
    //     AddrGlobal.CVX_BOOSTER.deposit(poolId, lpAmount, true); // 707 k
    //     vm.stopPrank();
    // }
    ...
    function test_cost_liquidation() external {
        ...
        vm.startPrank(usr1);
        lp.approve(address(AddrGlobal.CVX_BOOSTER), 1_000_000_000_000 ether);
        AddrGlobal.CVX_BOOSTER.deposit(poolId, lpAmount, true);

        uint256 price = curveLPOracle.getLPPrice(); // 56k
