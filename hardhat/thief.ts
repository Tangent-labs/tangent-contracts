...
export async function giveTokensToAddresses(
  users: Signer[],
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    for (let j = 0; j < tokensAmounts.length; j++) {
      ...
      if (tokenAmount.token.isVyper) {
        storageSlot = await GlobalHelper.calculateStorageSlotEthersVyper(
          userAddress,
          tokenAmount.token.slotBalance
        );
      } else {
        storageSlot = await GlobalHelper.calculateStorageSlotEthersSolidity(
          userAddress,
          tokenAmount.token.slotBalance
        );
      }
