    ...
    for (let i = 0; i < users.length; i++) {
        ...
        for (let j = 0; j < tokensAmounts.length; j++) {
            ...
            if (tokenAmount.token.isVyper) {
                storageSlot = GlobalHelper.calculateStorageSlotEthersVyper(userAddress, tokenAmount.token.slotBalance);
            } else {
                storageSlot = GlobalHelper.calculateStorageSlotEthersSolidity(userAddress, tokenAmount.token.slotBalance);
            }
