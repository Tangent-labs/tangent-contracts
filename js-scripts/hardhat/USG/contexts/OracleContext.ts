...
export class OracleContext {
    ...
    async deployChainlinkWrappers() {
        ...
        for (let index = 0; index < chainlinkOracleParams.length; index++) {
            ...
            this.oracles[item.key] = await ChainlinkWrapperFactory.deploy(await ethers.getContractAt("IPriceOracle", CHAINLINK_PRICE_FEEDS[item.oracleName]), 10000000000, ZeroAddress, item.oracleName);
        }
    }
