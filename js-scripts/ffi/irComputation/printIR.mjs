...
function printIR() {
    console.log(computeIR(USGPrice, isHEC, rMin, rMax, pMin, pInf, pMax, a1, a2, k));
}

function computeIR(USGPrice, isHEC, rMin, rMax, pMin, pInf, pMax, a1, a2, k) {
    const USGPriceNumber = Number(formatEther(BigInt(USGPrice)));
    const nomalizedPMin = Number(formatUnits(pMin, 6));
    const nomalizedPMax = Number(formatUnits(pMax, 6));
    if (USGPriceNumber <= nomalizedPMin) {
        return parseUnits(rMax, 13);
    }
    if (USGPriceNumber >= nomalizedPMax) {
        if (isHEC === "true") {
            return "0";
        }
        return parseUnits(rMin, 13);
    }
    const priceDelta = USGPriceNumber - Number(formatUnits(pInf, 6));
    ...
    const quotient = (nomalizedPMax - USGPriceNumber) / (nomalizedPMax - nomalizedPMin);
