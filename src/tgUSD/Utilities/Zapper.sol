...
//      */
//     function zapDeposit(ZapMarket calldata zapMarket, bool isStaked, address router, bytes calldata routerCall) external payable {
//         IMarketExternalActions(zapMarket.market).deposit(zapMarket._for, _zapDeposit(zapMarket, router, routerCall), isStaked);
//     }
...
//      *                        Otherwise, don't stake but the system takes a fee on the deposit.
//      */
//     function zapDepositAndBorrow(ZapMarket calldata zapMarket, uint256 tgUsdBorrowed, bool isStaked, address router, bytes calldata routerCall) external payable {
//         IMarketExternalActions(zapMarket.market).depositAndBorrow(_zapDeposit(zapMarket, router, routerCall), tgUsdBorrowed, isStaked, msg.sender);
//     }
...
//      *  @param routerCall  Raw data call used to call swap or swapMulti on the router Router
//      */
//     function zapRepay(ZapMarket calldata zapMarket, address router, bytes calldata routerCall) external payable {
//         IMarketExternalActions(zapMarket.market).repay(zapMarket._for, _zapRepay(zapMarket, router, routerCall), msg.sender);
//     }
...
//     function zapLeverage(IERC20 collatToken, uint256 minCollatReceived, address router, bytes calldata routerCall) external onlyMarket(msg.sender) returns (uint256) {
//         return _zapRouterAndVerify(collatToken, msg.sender, minCollatReceived, router, routerCall);
//     }

//     function zapCreateLock(IERC20 tokenIn, uint256 amountIn, uint256 minTanReceived, bool isPermaLock, address router, bytes calldata routerCall) external {
//         _transferTokenToZapper(tokenIn, amountIn, router);
//         rsTan.createLock(uint208(_zapRouterAndVerify(tan, address(rsTan), minTanReceived, router, routerCall)), isPermaLock, msg.sender);
//     }

//     function zapIncreaseLock(IERC20 tokenIn, uint256 amountIn, uint256 minTanReceived, uint256 tokenId, address router, bytes calldata routerCall) external {
//         _transferTokenToZapper(tokenIn, amountIn, router);
//         rsTan.increaseLockAmount(tokenId, uint208(_zapRouterAndVerify(tan, address(rsTan), minTanReceived, router, routerCall)), msg.sender);
//     }
...
//      *  @param routerCall  Raw data call used to call swap or swapMulti on the router Router
//      */
//     function _zapDeposit(ZapMarket calldata zapMarket, address router, bytes calldata routerCall) internal onlyMarket(zapMarket.market) returns (uint256) {
//         _transferTokenToZapper(zapMarket.tokenIn, zapMarket.amountIn, router);
//         return _zapRouterAndVerify(ICollateral(zapMarket.market).collatToken(), address(zapMarket.market), zapMarket.minAmountOut, router, routerCall);
//     }
...
//      *  @param routerCall  Raw data call used to call swap or swapMulti on the router Router
//      */
//     function _zapRepay(ZapMarket calldata zapMarket, address router, bytes calldata routerCall) internal onlyMarket(zapMarket.market) returns (uint256) {
...
//         return _zapRouterAndVerify(tgUSD, msg.sender, zapMarket.minAmountOut, router, routerCall);
//     }
...
//      */
//     function _transferTokenToZapper(IERC20 tokenIn, uint256 amountIn, address router) internal {
...
//      */
//     function _zapRouterAndVerify(IERC20 tokenOut, address receiver, uint256 minAmountOut, address router, bytes calldata routerData) internal returns (uint256) {
