...
contract RsTan is LightOwnable, ReentrancyGuardTransient, ERC721Enumerable, ZappingUtil {
   ...
   =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-= */
     ...
     * @param tokenId ID of the locking position
     */
    function unlock(uint256 tokenId, bool isClaimAsSgUSD) external nonReentrant onlyTokenOwner(tokenId) updateReward(tokenId) {
     ...
     */
    function claimSimple(uint256 tokenId, bool isClaimAsSgUSD) external nonReentrant onlyTokenOwner(tokenId) updateReward(tokenId) {
        bool isClaimable = _claimSimple(tokenId, msg.sender, isClaimAsSgUSD);
     ...
     */
    function claimMultiple(uint256[] calldata positionIds, bool isClaimAsSgUSD) external nonReentrant {
   ...
   =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-= */
    ...
    function _claimSimple(uint256 tokenId, address receiver, bool isClaimAsSgUSD) internal returns (bool) {
