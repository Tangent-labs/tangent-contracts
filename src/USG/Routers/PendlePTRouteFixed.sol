...
contract PendlePTRouterFixed is IPendlePTRouter {
    ...
    function swapTokenForPT(CurveRouterSwapNoReceiver calldata crvRouterData, PendleSYToPT calldata SYToPT) external payable returns (uint256) {
        ...
        uint256 underlyingAmount = crvRouterData._amount;

        // No Curve swap if the tokenIn is the underlyingIn
        if (SYToPT.underlyingIn != address(tokenIn)) {
            // Exchanges the tokenIn for one of the pendle market underlying
            underlyingAmount = _curveExchange(
