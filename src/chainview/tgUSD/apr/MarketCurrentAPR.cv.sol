...
contract MarketCurrentAPR {
    ...
    function getMarketTVLAPRs(address[] memory markets, IRewardAccumulator rewardAccumulator) public view returns (TVLAprs[] memory) {
        ...
        for (uint256 i; i < markets.length; i++) {
            ...
            uint256 totalStakedUSD = (oracle.latestAnswer() * totalStakedAmount) / 10 ** (18 - oracle.decimals());
            ...
            output[i] = TVLAprs({totalStakedAmount: totalStakedAmount, totalStakedUSD: totalStakedUSD, aprs: aprs});
        }
        return output;
    }
}
